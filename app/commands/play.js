...
const { YOUTUBE_API_KEY, SOUNDCLOUD_CLIENT_ID, DEFAULT_VOLUME } = require("../util/ZivobotUtil");
...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!args.length)
      return message
        .reply(`Usage: ${message.client.prefix}<YouTube URL'si | Video Adı | Soundcloud URL'si>`)
        .catch(console.error);
    ...
    if (mobileScRegex.test(url)) {
      try {
        https.get(url, function (res) {
          if (res.statusCode == "302") {
            ...
            return message.reply("Bu url'de içerik bulunamadı.").catch(console.error);
          }
        });
      } catch (error) {
        console.error(error);
      ...
      return message.reply("Aşağıdaki url yönlendirmesi...").catch(console.error);
    }
    ...
    if (serverQueue) {
      ...
      return serverQueue.textChannel
        .send(`✅ **${song.title}** Başarıyla ${message.author} tarafından sıraya eklendi!`)
        .catch(console.error);
    }
