...
const { YOUTUBE_API_KEY, SOUNDCLOUD_CLIENT_ID, MAX_PLAYLIST_SIZE, DEFAULT_VOLUME } = require("../util/ZivobotUtil");
...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!args.length)
      return message
        .reply(`Usage: ${message.client.prefix}oynatma listesi <YouTube Oynatma Listesi URL'si | Oynatma Listesi Adı>`)
        .catch(console.error);
    ...
    if (serverQueue && channel !== message.guild.me.voice.channel)
      return message.reply(`ile aynı kanalda olmalısınız ${message.client.user}`).catch(console.error);
    ...
    if (!serverQueue) {
      ...
      try {
        ...
        await channel.leave();
        return message.channel.send(`Kanala katılamadı: ${error.message}`).catch(console.error);
      }
    }
  }
};
